...
5. 成功后配置写入 `~/.agent-bridge/config.json`。
...
**聊天访问默认是私有的：开箱即用时，只有"你"能在私聊和群聊里用这个 bot。** 这里的"你" = 创建 / 拥有这个飞书应用的人（也就是扫码把 bot 建起来的那位）。bot 会自动从飞书查出谁是应用 owner，所以**一个人用聊天入口完全不用配置**——你私聊它、在任意群里 @它都正常工作，其他人的聊天消息会被静默忽略。云文档评论按文档权限生效，见下文。

### 三档群角色（REQ-03）

每个群可以独立配置角色，**角色跟群走**（一个人在不同群可以有不同角色）：

| 角色 | 标识 | 来源 | workspace 内 | workspace 外 |
|---|---|---|---|---|
| **Owner** | `owner` | 飞书应用创建者（运行时解析） | 读写+命令，全放开 | 读写+命令，全放开 |
| **讨论人** | `collaborator` | owner 用 `/role` 在当前群指定 | ✅ 读写+命令 | 受限 |
| **参与人** | `participant` | 群里其他人（按群策略） | ✅ 只读 | 受限 |

**管理角色**：
```
/role @某人 讨论人    # 在当前群把某人设为讨论人
/role @某人 参与人    # 设为参与人（仅读）
/role @某人 移除      # 移出角色名单
/role list            # 查看当前群角色配置
```

在**私聊**里发 `/config` 可选群做跨群管理。
...
| 策略 | 含义 |
|---|---|
| `strict`（默认） | 未被 `/role` 指定的人 → 静默拒绝 |
| `open-participant` | 群里所有人默认参与人（只读） |

在群里发 `/config` 可切换策略。

### 基础访问控制（兼容旧版）

想让别的同事或某些群也能用，也可以用旧的白名单方式：
...
> `/invite`、`/remove`、`/role` 这些命令只有**你（创建者）和管理员**能发。
...
- **只给自己用** → 什么都不用做，默认就是。
- **让某个同事能在群里协作写文件** → 在群里发 `/role @他 讨论人`
- **让某个同事能私聊 bot** → `/invite user @他`
- **让某个工作群里所有人都能只读** → 在群里发 `/config` 把群策略设为"开放只读"
- **再拉个人一起当管理员** → `/invite admin @他`

### 还需要知道的

- 改完**下一条消息**就生效，不用重启。
- **群里默认要先 @bot 才会回**（私聊不用 @）。这是另一个独立开关（`/config` →"群里需要 @ bot"）。
- 陌生人发消息一律静默丢弃，不会有任何回复。唯一的例外：有人在一个还没开放的群里 @bot，bot 会回一句友好提示。
- 云文档评论按文档权限生效：能在支持的文档里评论并 @bot 的人可以触发回复。

### 高级：直接改配置文件

`/role`、`/invite`、`/config` 背后写的是 `~/.agent-bridge/config.json` 中对应 profile 的 `access` 字段：
...
Agent-bridge 基于开源项目 [lark-channel-bridge](https://github.com/zarazhangrui/feishu-claude-code-bridge)（MIT 协议）二次开发，出处说明见 [NOTICE](./NOTICE)。
